...
import (
	model1 "Ozon_testtask/internal/model"
	"context"
	"errors"
	"fmt"
)
...
func (r *mutationResolver) AddPost(ctx context.Context, post model1.NewPost) (*model1.Post, error) {
...
func (r *mutationResolver) AddComment(ctx context.Context, comment model1.NewComment) (*model1.Post, error) {
	ctxWithTimeout, cancel := context.WithTimeout(ctx, TimeoutTime)
	defer cancel()
...
func (r *mutationResolver) ToggleComments(ctx context.Context, postID string, allowed bool) (*model1.Post, error) {
	ctxWithTimeout, cancel := context.WithTimeout(ctx, TimeoutTime)
	defer cancel()
...
func (r *queryResolver) Post(ctx context.Context, id string, limit int, offset int) (*model1.Post, error) {
	ctxWithTimeout, cancel := context.WithTimeout(ctx, TimeoutTime)
	defer cancel()
